...
for i in gene_name.index:
    util.update_progress(i, len(gene_name.index)) # check the time span
...
for i in gene_name_1.index:
    util.update_progress(i, len(gene_name_1.index))  # check the time span
...
for j in gene_name_1.index:
    util.update_progress(j, len(gene_name_1.index))  # check the time span
...
node_name.to_csv('node_name.csv',index=False) # save this data frame to csv
