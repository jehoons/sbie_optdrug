...
#         res = attr_cy.run(samples=100000, steps=60, debug=False, 
#                             on_states=on_states, off_states=off_states)
...
#     A,P,Q,U = attractor_summary(results[0])

#     print('A:',A,'P:',P, 'Q:', Q,'U:', U)
...
def test_summary_32results():
    ...
    # fobj = open(file_s1b,'w') 
    # fobj.write('input,phenotype,ratio\n')
...
# output 
file_b1 = dirname(tab_s1.__file__) + '/b/b1-simul-result-with-32-conditions.json'
file_b2 = dirname(tab_s1.__file__) + '/b/b2-simul-result-table-summary.csv'

with open(file_a2, 'r') as fobj:
    modeltext = "".join(fobj.readlines())
